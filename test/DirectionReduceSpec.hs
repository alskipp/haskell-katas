...
dirReduce = toList . foldl' f Seq.empty . Seq.fromList
  where
    ...
    isOpposite s x = maybe False (op x) (last s)
...
spec :: Spec
spec =
  describe "dirReduce - simple tests"
    $ it "should work for some small examples"
    $ do
      dirReduce [] `shouldBe` []
      dirReduce [North] `shouldBe` [North]
      dirReduce [North, West] `shouldBe` [North, West]
      dirReduce [North, West, East] `shouldBe` [North]
      dirReduce [North, West, South, East]
        `shouldBe` [North, West, South, East]
